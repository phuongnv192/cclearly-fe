...
import { useLensesStore } from '@stores';

/**
 * Lenses page component
...
const LensesPage = () => {
  ...
  const {
    ...
    toggleLensType,
    toggleTechnology,
    toggleRefractiveIndex,
    setPriceRange,
    clearFilters,
    setCurrentPage,
  } = useLensesStore();

  // Fetch lenses on mount and when filters/pagination changes
  useEffect(() => {
    fetchLenses();
  }, [
    fetchLenses,
  ...
  const handleRefractiveIndexChange = useCallback(
    (index) => {
        ...
        <div className="container mx-auto px-4 py-12 flex flex-col md:flex-row items-center justify-between">
          ...
          <div className="w-full md:w-1/2 mt-8 md:mt-0 text-center md:text-left">
            ...
            <p className="text-lg text-slate-600 dark:text-slate-400 max-w-md mx-auto md:mx-0">
              Tròng kính chất lượng cao với đa dạng tính năng và chỉ số chiết suất, 
              bảo vệ đôi mắt của bạn một cách tối ưu.
            </p>
          </div>
        </div>
                ...
                <div className="space-y-2">
                  {lensTypes.map((type) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={filters.lensTypes.includes(type.id)}
                        onChange={() => handleLensTypeChange(type.id)}
                        className="rounded border-slate-300 text-primary focus:ring-primary h-4 w-4"
                      />
                      <span
                        className={`ml-3 text-sm transition-colors ${
                          filters.lensTypes.includes(type.id)
                ...
                <div className="space-y-2">
                  {technologies.map((tech) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={filters.technologies.includes(tech.tech_id)}
                        onChange={() => handleTechnologyChange(tech.tech_id)}
                        className="rounded border-slate-300 text-primary focus:ring-primary h-4 w-4"
                      />
                      <span
                        className={`ml-3 text-sm transition-colors ${
                ...
                <div className="space-y-2">
                  {refractiveIndexes.map((idx) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={filters.refractiveIndexes.includes(idx.id)}
                        onChange={() => handleRefractiveIndexChange(idx.id)}
                        className="rounded border-slate-300 text-primary focus:ring-primary h-4 w-4"
                      />
                      <span
                        className={`ml-3 text-sm transition-colors ${
                          filters.refractiveIndexes.includes(idx.id)
                            ? 'font-medium text-primary'
                            : 'text-slate-600 dark:text-slate-400 group-hover:text-primary'
                        }`}
                      >
