...
const FramesPage = () => {
  ...
  // Fetch frames on mount and when filters/pagination changes
  useEffect(() => {
    fetchFrames();
  }, [
    fetchFrames,
      ...
      <section className="relative bg-slate-50 dark:bg-slate-900 overflow-hidden">
        <div className="container mx-auto px-4 py-12 flex flex-col md:flex-row items-center justify-between">
          <div className="w-full md:w-1/2 flex justify-center relative">
            <img
              alt="Gọng Kính Cận"
              className="max-w-md rounded-xl shadow-2xl z-10 relative"
              src="https://images.unsplash.com/photo-1574258495973-f010dfbb5371?w=600"
            />
            <div className="absolute -bottom-6 -left-6 w-48 h-48 bg-teal-400 rounded-full mix-blend-multiply opacity-20 blur-xl"></div>
          </div>
          <div className="w-full md:w-1/2 mt-8 md:mt-0 text-center md:text-right">
            ...
            <p className="text-lg text-slate-600 dark:text-slate-400 max-w-md ml-auto">
              Phong cách tối giản, khẳng định cá tính riêng.
            </p>
          </div>
        </div>
        <div className="absolute top-10 right-10 text-primary opacity-20">
          <span className="material-icons text-8xl">eyeglasses</span>
        </div>
      </section>
                ...
                <div className="space-y-2">
                  {frameWidths.map((width) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={filters.widths.includes(width.id)}
                        onChange={() => handleWidthChange(width.id)}
                        className="rounded border-slate-300 text-primary focus:ring-primary h-4 w-4"
                      />
                      <span
                        className={`ml-3 text-sm transition-colors ${
                          filters.widths.includes(width.id)
