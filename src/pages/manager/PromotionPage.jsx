...
const PromotionPage = () => {
  const { data: coupons = [] } = usePromotions();
  ...
  const handleCouponSubmit = (e) => {
    ...
    if (editingCoupon) {
      updatePromotionMutation.mutate(
        { id: editingCoupon.id || editingCoupon.code, data: couponFormData },
        {
          onSuccess: () => {
      ...
      createPromotionMutation.mutate(couponFormData, {
        onSuccess: () => {
          setShowCouponModal(false);
          setEditingCoupon(null);
        },
  ...
  const toggleCouponStatus = (coupon) => {
    togglePromotionMutation.mutate(coupon.id || coupon.code);
  };
  ...
  const handleFlashSaleSubmit = (e) => {
    ...
    const product = allProducts.find(
      (p) => p.id === flashSaleFormData.productId || p.productId === flashSaleFormData.productId
    );
  ...
  const onConfirmDelete = () => {
    if (confirmDelete.type === 'coupon') {
      ...
      setFlashSales((prev) => prev.filter((fs) => fs.id !== confirmDelete.id));
  ...
  const filteredCoupons = coupons.filter((c) => {
    ...
    if (couponFilter === 'expired') return matchesSearch && !c.isActive;

    return matchesSearch;
  });

  const activeFlashSales = flashSales.filter((fs) => fs.slot === activeSlot);
    ...
    <div className="p-8 space-y-8 max-w-[1400px] mx-auto">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 text-sm">Quản lý Flash Sale và Voucher</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-5">
        ...
        <Stat icon={Zap} label="Flash Sale" value={flashSales.length} />
        ...
        <Stat icon={Clock} label="Today" value={124} />
      </div>
        ...
        <div className="col-span-4 bg-white border rounded-xl p-5 space-y-4">
          ...
          <div className="space-y-3">
            {activeFlashSales.map((fs) => {
              const percent = (fs.sold / fs.stock) * 100;

              return (
                <div key={fs.id} className="border rounded-lg p-3 space-y-2">
                  <div className="flex justify-between">
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => openEditFlashSale(fs)}>
                        <Edit2 size={14} />
                      </button>
              ...
              {filteredCoupons.map((coupon) => (
                <tr key={coupon.code} className="border-t">
                  ...
                  <td className="p-3 text-right">
                    ...
                    <button
                      onClick={() => handleDeleteRequest('coupon', coupon.id || coupon.code)}
                      className="text-red-600"
                    >
                      <Trash2 size={16} />
                    </button>
