...
const OperationsLensLabPage = () => {
  ...
  const prescriptionOrders = orders.filter(
    (o) =>
      o.prescription &&
      ['confirmed', 'processing'].includes(o.status?.toLowerCase())
  );
  ...
  const handleStartWork = (orderId) => {
    updateStatusMutation.mutate(
      ...
      {
        onSuccess: () => toast.success(`Đã bắt đầu gia công cho đơn ${orderId}`),
      }
    );
  };
  ...
  const handleQC = (orderId, pass) => {
    if (pass) {
      updateStatusMutation.mutate(
        ...
        {
          onSuccess: () => toast.success(`Đơn ${orderId} đã qua QC - sẵn sàng đóng gói`),
        }
      );
    } else {
  ...
  const confirmQCFail = () => {
    if (qcFailModal.reason) {
      toast.error(
        `Đơn ${qcFailModal.orderId} fail QC - ${qcFailModal.reason}. Yêu cầu làm lại.`
      );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white rounded-xl p-4 shadow-sm">
          <p className="text-2xl font-bold text-blue-600">
            {prescriptionOrders.filter((o) => o.status?.toLowerCase() === 'confirmed').length}
          </p>
          <p className="text-sm text-[#4f5562]">Chờ gia công</p>
        </div>
        <div className="bg-white rounded-xl p-4 shadow-sm">
          <p className="text-2xl font-bold text-purple-600">
            {prescriptionOrders.filter((o) => o.status?.toLowerCase() === 'processing').length}
          </p>
          <p className="text-sm text-[#4f5562]">Đang gia công</p>
        </div>
        <div className="bg-white rounded-xl p-4 shadow-sm">
          <p className="text-2xl font-bold text-green-600">
            {prescriptionOrders.filter((o) => o.status?.toLowerCase() === 'completed').length}
          </p>
      ...
      <div className="space-y-4">
        {prescriptionOrders.map((order) => {
          const statusBadge = getStatusBadge(order.status?.toLowerCase());
          return (
            <div key={order.orderId || order.id} className="bg-white rounded-xl p-6 shadow-sm">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-[#222]">{order.code || order.orderId || order.id}</span>
              ...
              {order.status?.toLowerCase() === 'confirmed' && (
                <button
                  onClick={() => handleStartWork(order.orderId || order.id)}
              ...
              {order.status?.toLowerCase() === 'processing' && (
                <div className="flex gap-2">
                  <button
                    onClick={() => handleQC(order.orderId || order.id, true)}
                  ...
                  <button
                    onClick={() => handleQC(order.orderId || order.id, false)}
