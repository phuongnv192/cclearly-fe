              ...
              {column.orders.map((order) => (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs ${getStatusBadge(order.status).class}`}
                    >
                      {getStatusBadge(order.status).label}
                    </span>
                  </div>
                  <p className="text-sm text-[#4f5562] mb-2">
                  ...
                  <div className="mt-3 pt-3 border-t">
                    ...
                    {column.id === 'processing' && (
                      <button
                        onClick={() => handleMoveOrder(order.orderId || order.id, 'processing')}
                    ...
                    {column.id === 'packaging' && (
                      <button
                        onClick={() =>
                          handleMoveOrder(order.orderId || order.id, 'shipped', 'Đang giao')
                        }
