...
import { useAdminUsers, useAdminLogs, useCreateUser, useUpdateUser, useDeleteUser } from '@/hooks/useAdmin'
...
const RolePermissionPage = () => {
    ...
    const { data: usersData, isLoading: loadingUsers } = useAdminUsers({ page: 0, size: 50 })
    const { data: logsData, isLoading: loadingLogs } = useAdminLogs({ page: 0, size: 20 })
    const createUserMutation = useCreateUser()
    const updateUserMutation = useUpdateUser()
    const deleteUserMutation = useDeleteUser()

    const employees = Array.isArray(usersData) ? usersData : usersData?.content || []
    const auditLogs = logsData?.items || []

    // Count users per role
    const roleCounts = employees.reduce((acc, u) => {
    ...
    return (
        <div className="p-8 space-y-8">
            ...
            <div className="grid md:grid-cols-3 gap-6">
                {[
                    { icon: Shield, title: 'Admin', desc: 'Toàn quyền hệ thống', count: roleCounts.admin, color: 'bg-red-50 text-red-600' },
                    { icon: User, title: 'Manager', desc: 'Quản lý kho và báo cáo', count: roleCounts.manager, color: 'bg-blue-50 text-blue-600' },
                    { icon: Settings, title: 'Staff', desc: 'Xử lý đơn hàng & Sales', count: roleCounts.staff, color: 'bg-green-50 text-green-600' },
                ].map((item, i) => (
                    ...
                    {activeTab === 'accounts' ? (
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {employees.map((emp) => (
                                        <tr key={emp.userId} className="hover:bg-gray-50 transition">
                                            <td className="px-4 py-4 font-mono text-sm text-[#666]">{emp.email}</td>
                                            <td className="px-4 py-4 font-bold text-[#222]">{emp.fullName}</td>
                                            ...
                                            <td className="px-4 py-4 text-[#666] italic text-xs">{emp.lastLogin ? new Date(emp.lastLogin).toLocaleString('vi-VN') : 'Chưa đăng nhập'}</td>
                                            <td className="px-4 py-4 text-right space-x-2">
                                                <button onClick={() => setEditingUser(emp)} className="text-[#0f5dd9] font-bold hover:underline">Sửa</button>
                                                <button onClick={() => handleDelete(emp.userId)} className="text-red-500 font-bold hover:underline">Xóa</button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {auditLogs.map((log) => (
                                        <tr key={log.logId} className="hover:bg-gray-50 transition text-xs">
                                            <td className="px-4 py-4 font-bold text-[#222]">{log.userName}</td>
                                            ...
                                            <td className="px-4 py-4 font-medium text-[#4f5562]">{log.details}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
